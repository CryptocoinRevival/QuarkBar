...
const QString BITCOIN_IPC_PREFIX("quarkbar:");
...
{
    // Install global event filter to catch QFileOpenEvents on the mac (sent when you click quarkbar: links)
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start quarkbar: click-to-pay handler");
    else
...
{
    // clicking on quarkbar: URLs creates FileOpen events on the Mac:
