...
uint256 hashGenesisBlock("0x00000cfd0dd51bc3150dba1567ebabfee6010f6ce3fce59078b2cee81689902b");
static const unsigned int timeGenesisBlock = 1393772400; //Sun, 02 Mar 2014 15:00:00 GMT
...
int64 static GetBlockValue(int nHeight, int64 nFees, unsigned int nBits)
{
	...
	else if(nHeight < 2881)
	{
		return 2 * nBlockRewardStartCoin;
	}
    
    int64 nSubsidy = nBlockRewardStartCoin;

    // Subsidy is cut in half every 40320 blocks (14 days)
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
	if( fTestNet ) {
		...
		if( pindexLast->nHeight+1 >= 232500 ) {
			return DarkGravityWave3(pindexLast, pblock);
...
//                           (pindex->nHeight==91880 && pindex->GetBlockHash() == uint256("0x00000000000743f190a18c5577a3c2d2a1f610ae9601ac046a38084ccb7cd721")));
    ...
    if (vtx[0].GetValueOut() > GetBlockValue(pindex->nHeight, nFees, pindex->nBits) && pindex->nHeight != MIN_SUPPLY)
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis block
        const char* pszTimestamp = "Russian troops take over Ukraine's Crimea region";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nNonce   = 12242800;

        if (fTestNet)
        {
            block.nTime    = 1393772400; //Sun, 02 Mar 2014 15:00:00 GMT
            block.nNonce   = 0;
        }
        ...
        block.print();
        assert(block.hashMerkleRoot == uint256("0x068d295054fe29b066c79bb13869bf1ccfd738d9a8fc19741d2beccc16950ded"));
